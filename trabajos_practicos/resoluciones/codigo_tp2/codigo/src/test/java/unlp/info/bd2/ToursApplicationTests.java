...
class ToursApplicationTests {
	...
	void updateUserTest()  throws ToursException {
		...
		assertTrue(opUnmodifiedUserFromDB.isEmpty());
		User updatedUserFromDB = opUserFromDB.get();
		assertEquals(updatedUserFromDB.getId(), user1.getId());
		assertEquals("user2", updatedUserFromDB.getUsername());
	}
	...
	void deleteUserTest() throws ToursException {
		...
		assertTrue(optionalUser2.isEmpty());
